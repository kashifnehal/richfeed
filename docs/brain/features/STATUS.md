...
| Accounts | `/(dashboard)/accounts` | live data | Per-status badges (connected / needs_reconnect / limited / disconnected). X (Twitter), Instagram, Facebook (via a Page picker screen), and Threads all connect via real OAuth (see `platforms/x.md`, `platforms/instagram.md`, `platforms/facebook.md`, `platforms/threads.md`); every other platform still shows honest "coming in the next build" messaging. Disconnect is a soft status change (`disconnected`), not a delete — history stays intact; "Remove permanently" (disconnected accounts only) is blocked while any post_targets still reference the account (see `DECISIONS.md` 2026-09-03). |
