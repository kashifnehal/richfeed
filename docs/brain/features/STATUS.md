...
_Last updated: 2026-09-18 (E2E claims removed — Playwright suite deleted
2026-09-03, commit `c78fc47`, and must not be recreated). Determined by
...
| Compose | `/(dashboard)/posts/new` | live data | Media upload to Supabase Storage, hashtag input, account multi-select (needs_reconnect accounts shown but disabled with inline reason; disconnected accounts hidden entirely), per-target scheduling + caption override, live per-platform preview, "select at least one account" guard. `media_type` inferred from upload count, not MIME. **Known gap:** no pre-schedule media-type validation against the per-platform capability matrix — a user can schedule a video to LinkedIn/Facebook/Threads/X (or text-only to Instagram/YouTube) and only learn it failed at publish time. |
