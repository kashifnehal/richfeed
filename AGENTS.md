...
## 5. Currently open, real bugs (as of 2026-09-05 — confirm these are still open before assuming so; nothing had shipped past commit `2716e3d` as of 2026-09-17)

1. **No pre-schedule media-type validation.** The compose/schedule UI and its backing API route let a user schedule any media type to any target, even when that adapter's `assertSupportedMedia()` will reject it at publish time. Fix: validate against the real capability matrix above at schedule time (both client-side UX and a server-side guard, since the server must not trust the client), and surface a clear inline error rather than letting the post fail silently later.
2. **Instagram video posting returns a real 400 "Invalid parameter" from Meta.** Root cause unknown — `buildMetaError` in `apps/api/src/platforms/meta-shared.ts` currently only surfaces Meta's short error message, not the full response body, so the exact rejected field has never been seen. Fix step one is to log the full Meta error response body, then diagnose from there. Do not guess at a fix without seeing the real error body first.
3. **YouTube video upload returns a real 401 "invalid authentication credentials."** This is a *separate* bug from an earlier, already-fixed connect-flow issue (a scope bug fixed in commit `a8ce11e`/`2716e3d` — the OAuth *connect* step works fine now). This 401 happens later, during the actual upload call in `apps/api/src/platforms/youtube.ts`'s `publishToYouTube()`. On a structural read-through, `getValidAccessToken()` / `refreshAccessToken()` / the `Authorization: Bearer` header construction all looked correct — the root cause was not found by inspection alone. `throwYouTubeError()` currently only surfaces `body.error.message`, not the full response body. Fix step one is the same as #2: log the full error body from the upload-init call, then investigate token freshness and header construction from real evidence, not assumption.

Treat #1 as the highest-leverage fix (it prevents the other failure modes from ever reaching a real user silently) and #2/#3 as real platform-integration bugs that need actual error-body evidence before attempting a fix.

## 6. House rules (non-negotiable — each one exists because of a real incident)

- **Never write or reintroduce automated tests.** The Playwright E2E suite was deliberately deleted (commit `4c87b54`) as a founder decision. Verify manually / by reading real logs and DB state instead. If you use a Playwright MCP tool for ad-hoc verification during a session, that's fine — just don't check in a test suite.
